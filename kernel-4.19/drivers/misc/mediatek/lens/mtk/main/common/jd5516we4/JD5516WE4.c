...
/*
 * JD5516WE4 voice coil motor driver
 *
 *
 */
...
#define AF_DRVNAME "JD5516WE4_DRV"
...
static int s4JD5516WE4_ReadReg(unsigned short *a_pu2Result)
{
...
/* ////////////////////////////////////////////////////////////// */
long JD5516WE4_Ioctl_Main(struct file *a_pstFile,
...
int JD5516WE4_Release_Main(struct inode *a_pstInode, struct file *a_pstFile)
{
    LOG_INF("Start\n");
    ...
    if (g_u4CurrPosition > g_u4AF_INF && g_u4CurrPosition <= g_u4AF_MACRO) {
        while (g_u4CurrPosition > 512) {
            ...
            if (s4AF_WriteReg(g_u4CurrPosition) != 0) {
                LOG_INF("AF JD5516WE4_Release I2C failed");
            }
            LOG_INF("JD5516WE4_Release pos %d", g_u4CurrPosition);
            udelay(1000);
        }
            s4AF_WriteReg(512);
            udelay(1000);
...
int JD5516WE4_SetI2Cclient_Main(struct i2c_client *pstAF_I2Cclient,
		spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
	...
	if (*g_pAF_Opened == 1) {
		initdrv();
		ret = s4JD5516WE4_ReadReg(&InitPos);
...
int JD5516WE4_GetFileName_Main(unsigned char *pFileName)
{
