 ...
 * Filename:
 * ---------
 *	 atom_shinetech_front_s5k5e9yx04_mipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "atom_s5k5e9yx04mipiraw_Sensor.h"

#define PFX "ATOM_s5k5e9yx_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = ATOM_S5K5E9YX04_SENSOR_ID,
...
{
	return (((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001)) + 2);
}
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			printk("wenhui s5k5e9yxmipiraw_Sensor open sensor_id = 0x%x",sensor_id);
			if (sensor_id == ATOM_S5K5E9YX04_SENSOR_ID) {
