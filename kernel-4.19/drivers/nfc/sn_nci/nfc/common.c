...
int nfcc_hw_check(struct nfc_dev *nfc_dev)
{
        int ret = 0;
        struct platform_gpio *nfc_gpio = &nfc_dev->configs.gpio;
        uint8_t *nci_get_fw_cmd = nfc_dev->write_kbuf;
        uint8_t *nci_get_fw_rsp = nfc_dev->read_kbuf;
    ...
    #define GET_VERSION_RSP_MINOR_VERSION_OFFSET	6
        ...
        if (ret <= 0) {
            pr_err("%s: - - nfc get version cmd error ret\n", __func__);
            goto err;
        }

        memset(nci_get_fw_rsp, 0x00, DL_GET_VERSION_RSP_LEN_2);
        /* Read Response of FW Version CMD */
        ret = nfc_dev->nfc_read(nfc_dev, nci_get_fw_rsp, DL_GET_VERSION_RSP_LEN_2, NCI_CMD_RSP_TIMEOUT_MS);
        if (ret <= 0) {
            pr_err("%s: - nfc get version rsp error ret %d\n", __func__, ret);
            goto err;
        }

        if (nci_get_fw_rsp[0] == FW_MSG_CMD_RSP && ret >= DL_GET_VERSION_RSP_LEN_2) {
            pr_debug("%s:NFC Chip Type 0x%02x Rom Version 0x%02x FW Minor 0x%02x Major 0x%02x\n",
                     __func__, nci_get_fw_rsp[GET_VERSION_RSP_CHIP_TYPE_OFFSET],
                               nci_get_fw_rsp[FW_ROM_CODE_VER_OFFSET],
                               nci_get_fw_rsp[GET_VERSION_RSP_MINOR_VERSION_OFFSET],
                               nci_get_fw_rsp[FW_MAJOR_VER_OFFSET]);

            nfc_dev->nqx_info.info.chip_type = nci_get_fw_rsp[GET_VERSION_RSP_CHIP_TYPE_OFFSET];
            nfc_dev->nqx_info.info.rom_version = nci_get_fw_rsp[FW_ROM_CODE_VER_OFFSET];
            nfc_dev->nqx_info.info.fw_minor = nci_get_fw_rsp[GET_VERSION_RSP_MINOR_VERSION_OFFSET];
            nfc_dev->nqx_info.info.fw_major = nci_get_fw_rsp[FW_MAJOR_VER_OFFSET];
        }
