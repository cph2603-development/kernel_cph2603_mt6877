...
#include "../mediatek/mtk_corner_pattern/ac144_p_7_a0012_data_hw_roundedpattern.h"
...
static int get_mode_enum(struct drm_display_mode *m)
{
	...
	if (m == NULL) {
		DISP_DEBUG("get_mode_enum drm_display_mode *m is null ,default 60fps\n");
		ret = 60;
		return ret;
	}

		m_vrefresh = drm_mode_vrefresh(m);
		DISP_DEBUG("m_vrefresh=%d\n", m_vrefresh);
		if (m_vrefresh == 60) {
			ret = 60;
				 ...
				 unsigned int level)
{
	...
	DISP_DEBUG("backlight=%d,paralist[1]=0x%x,paralist[2]=0x%x\n", level, bl_level[1].para_list[1], bl_level[1].para_list[2]);
			      ...
			      dcs_write_gce cb, void *handle, unsigned int level)
{
	...
	bl_level[0].para_list[1] = BL_LSB;
	bl_level[0].para_list[2] = BL_MSB;
...
{
	struct lcm *ctx = panel_to_lcm(panel);
	DISP_DEBUG("oplus_send_cmd_before_dsi_read_ili\n");
	lcm_dcs_write_seq_static(ctx, 0xFF, 0x78, 0x38, 0x0F);
	return 0;
}
...
{
	struct lcm *ctx = panel_to_lcm(panel);
	DISP_DEBUG("oplus_send_cmd_after_dsi_read\n");
	lcm_dcs_write_seq_static(ctx, 0xFF, 0x78, 0x38, 0x00);
	return 0;
}
...
static int vddr6_buck4_regulator_init(struct device *dev)
{
    ...
    DISP_DEBUG("get vddr6_buck4_regulator_init +\n");
...
static int vddr6_buck4_regulator_enable(struct device *dev)
{
	int ret = 0;
	int retval = 0;
	DISP_DEBUG("get vddr6_buck4_regulator_enable +\n");
...
static int vddr6_buck4_regulator_disable(struct device *dev)
{
	int ret = 0;
	int retval = 0;
   	DISP_DEBUG("get vddr6_buck4_regulator_disable +\n");
	vddr6_buck4_regulator_init(dev);
	if (!IS_ERR_OR_NULL(mt6315_6_vbuck4)) {
...
static int lcm_panel_poweron(struct drm_panel *panel)
{
	...
	DISP_DEBUG("p_7 a0012 lcm ctx->prepared %d\n", ctx->prepared);
