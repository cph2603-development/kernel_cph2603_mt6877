...
static int panel_doze_disable(struct drm_panel *panel, void *dsi, dcs_write_gce cb, void *handle)
{
	...
	pr_err("debug for lcm\n");
       ...
       if (local_lcm_ctx->hbm_en) {
               ...
               //lcm_dcs_write_seq_static(local_lcm_ctx, 0x28);^M
               //msleep(17);^M
		...
		void *handle, unsigned int level)
{
	...
	char bl_tb2[] = {0xF0, 0x5A, 0x5A};
	char bl_tb3[] = {0x51, 0x07, 0xFF};
	char bl_tb4[] = {0x91, 0x02, 0x00};
	char bl_tb5[] = {0xF0, 0xA5, 0xA5};
	pr_info("level = %d\n", level);
	if (!cb)
		return -1;
	if (level ==1) {
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "oplus20615_samsung_ams643ye01_evt_1080p_dsi_cmd", },
	{ }
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "oplus20615_samsung_ams643ye01_evt_1080p_dsi_cmd",
		.owner = THIS_MODULE,
