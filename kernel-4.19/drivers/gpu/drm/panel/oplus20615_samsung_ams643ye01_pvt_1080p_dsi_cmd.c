...
static struct LCM_setting_table lcm_setbrightness_normal_20615[] = {
        {REGFLAG_CMD,2, {0x53,0x20}},
		{REGFLAG_CMD,3, {0x51,0x00,0x00}},
        //{REGFLAG_CMD,3, {0xF0, 0x5A, 0x5A}},
        //{REGFLAG_CMD,2, {0xB0,0x01}},
...
static struct LCM_setting_table lcm_setbrightness_hbm_20615[] = {
        {REGFLAG_CMD,2, {0x53,0xE8}},
	{REGFLAG_CMD,3, {0x51,0x00,0x00}},
        {REGFLAG_END_OF_TABLE, 0x00, {}}
};
...
static struct LCM_setting_table lcm_finger_HBM_on_setting_20615[] = {
        {REGFLAG_CMD,2, {0x53,0xE0}},
        {REGFLAG_CMD,3, {0x51,0x0A, 0xE8}},
	//{REGFLAG_CMD,3, {0xF0, 0x5A, 0x5A}},
        //{REGFLAG_CMD,3, {0xB0,0x01}},
...
//	pr_info("level is %d\n", level);
	...
	if (level == 1) {
		...
		if (is_project(20615)) {
			pr_info("the project is 20615, level = %d\n", level);
			lcm_setbrightness_hbm_20615[1].para_list[1] = BL_LSB;
			lcm_setbrightness_hbm_20615[1].para_list[2] = BL_MSB;
			for (i = 0; i < sizeof(lcm_setbrightness_hbm_20615)/sizeof(struct LCM_setting_table); i++){
				for (i = 0; i < sizeof(lcm_setbrightness_hbm_20615)/sizeof(struct LCM_setting_table); i++){
					cb(dsi, handle, lcm_setbrightness_hbm_20615[i].para_list, lcm_setbrightness_hbm_20615[i].count);
				}
			}
		} else {
			pr_info("the project is other, level = %d\n", level);
		...
		if (is_project(20615))	{
			pr_info("the project is 20615, level = %d\n", level);
			lcm_setbrightness_normal_20615[1].para_list[1] = BL_LSB;
			lcm_setbrightness_normal_20615[1].para_list[2] = BL_MSB;
			for (i = 0; i < sizeof(lcm_setbrightness_normal_20615)/sizeof(struct LCM_setting_table); i++){
				cb(dsi, handle, lcm_setbrightness_normal_20615[i].para_list, lcm_setbrightness_normal_20615[i].count);
			}
		} else {
		    pr_info("the project is other, level = %d\n", level);
		...
		void *handle, unsigned int hbm_mode)
{
	...
	if(hbm_mode == 1) {
		...
		if (is_project(20615)) {
			for (i = 0; i < sizeof(lcm_finger_HBM_on_setting_20615)/sizeof(struct LCM_setting_table); i++){
				cb(dsi, handle, lcm_finger_HBM_on_setting_20615[i].para_list, lcm_finger_HBM_on_setting_20615[i].count);
			}
		} else {
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "oplus20615_samsung_ams643ye01_pvt_1080p_dsi_cmd", },
	{ }
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "oplus20615_samsung_ams643ye01_pvt_1080p_dsi_cmd",
		.owner = THIS_MODULE,
