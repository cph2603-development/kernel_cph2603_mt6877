...
struct typec_switch_priv {
	...
	/*2023/02/27, supporting type-c headphone detect bypass*/
	bool hp_bypass;
};
		...
		u32 switch_control, u32 switch_enable)
{
	...
	regmap_write(switch_priv->regmap, DEFAULT_REG_SWITCH_SETTINGS, reg_val);
	regmap_write(switch_priv->regmap, DEFAULT_REG_SWITCH_SELECT, switch_control);
...
int typec_switch_to_fast_charger(int to_fast_charger)
{
	...
	if (to_fast_charger) {
		if (switch_priv->vendor == DIO4483) {
			...
			regmap_write(switch_priv->regmap, DIO4483_REG_SWITCH_SELECT, reg_val);
			...
			regmap_write(switch_priv->regmap, DIO4483_REG_SWITCH_SETTINGS, reg_val);
			...
			ret = regmap_write(switch_priv->regmap, DEFAULT_REG_SWITCH_SELECT, reg_val);
			if (ret != 0) {
				goto regmap_err;
			}
			...
			ret = regmap_write(switch_priv->regmap, DEFAULT_REG_SWITCH_SETTINGS, reg_val);
			if (ret != 0) {
				goto regmap_err;
			}
			usleep_range(10000, 10005);
...
regmap_err:
	...
	mm_fb_audio_fatal_delay(10047, MM_FB_KEY_RATELIMIT_5MIN, \
				FEEDBACK_DELAY_60S, "payload@@regulator typec_switch failed");
#endif /*CONFIG_OPLUS_FEATURE_MM_FEEDBACK*/
	reg_val = 0;
	SET_BIT(reg_val, DEFAULT_I2C_RESET);
	regmap_write(switch_priv->regmap, DEFAULT_REG_RESET, reg_val);//reset
	return ret;
}
...
int typec_switch_status0(void)
{
	...
	if (!switch_priv) {
		pr_err("%s, switch_priv is NULL", __func__);
		SET_BIT(rc, TYPEC_AUDIO_SWITCH_STATE_INVALID_PARAM);
		goto err_handler;
	}
	...
	if ((dn_l_status == 0x01) && (dp_r_status == 0x01)) { // Charger
		CLEAR_BIT(rc, TYPEC_AUDIO_SWITCH_STATE_DPDM);
	} else if ((dn_l_status == 0x03) && (dp_r_status == 0x03)) { // Fast Charger
		SET_BIT(rc, TYPEC_AUDIO_SWITCH_STATE_FAST_CHG);
	} else if ((dn_l_status == 0x02) && (dp_r_status == 0x02)) { // Headphone
		SET_BIT(rc, TYPEC_AUDIO_SWITCH_STATE_AUDIO);
	} else { // Unknown
		SET_BIT(rc, TYPEC_AUDIO_SWITCH_STATE_UNKNOW);
		pr_err("%s, Typec audio switch state is unknow", __func__);
	}

err_handler:
	if (err_status) {
		SET_BIT(rc, TYPEC_AUDIO_SWITCH_STATE_I2C_ERR);
...
static int typec_switch_usbc_analog_setup_switches(struct typec_switch_priv *switch_priv)
{
	...
	if (switch_priv->plug_state) {
		...
		if (switch_priv->vendor == DIO4480) {
			/* activate switches */
			ret = regmap_write(switch_priv->regmap, DIO4480_REG_RESET, 0x01);//reset DIO4480
			if (ret != 0) {
				dev_err(dev, "%s, %d, regmap fail!\n", __func__, __LINE__);
				goto regmap_err;
			}
			usleep_range(1000, 1005);
			ret = regmap_write(switch_priv->regmap, DIO4480_REG_FUN_EN, 0x45);
			if (ret != 0) {
				dev_err(dev, "%s, %d, regmap fail!\n", __func__, __LINE__);
				goto regmap_err;
			}
			...
			regmap_write(switch_priv->regmap, DIO4483_REG_RESET, reg_val); // reset DIO4483
			usleep_range(1000, 1005);
			...
			dev_info(dev, "%s, %d, write 0x%02x = 0x%02x", __func__, __LINE__, DIO4483_REG_FUN_EN, reg_val);
			regmap_write(switch_priv->regmap, DIO4483_REG_FUN_EN, reg_val);
			...
			typec_switch_status = 0;
		} else if (switch_priv->vendor == FSA4480) {
			...
			dev_info(dev, "%s, %d, set reg[0x%x] done.\n", __func__, DEFAULT_REG_FUN_EN);
			typec_switch_status = 0;
		} else {
			reg_val = 0x00;
			dev_info(dev, "%s, %d, write 0x%02x = 0x%02x", __func__, __LINE__, DEFAULT_REG_SWITCH_SELECT, reg_val);
			...
			ret = regmap_write(switch_priv->regmap, 0x19, reg_val);
			if (ret != 0) {
				dev_err(dev, "%s, %d, regmap fail!\n", __func__, __LINE__);
				goto regmap_err;
			}
			...
			ret = regmap_write(switch_priv->regmap, DEFAULT_REG_FUN_EN, reg_val);
			if (ret != 0) {
				dev_err(dev, "%s, %d, regmap fail!\n", __func__, __LINE__);
				goto regmap_err;
			}
		...
		if (switch_priv->vendor == DIO4480) {
			...
			if (jack_status == 0x01) {
				...
				regmap_write(switch_priv->regmap, DIO4480_REG_RESET, 0x01);//reset DIO4480
				usleep_range(1000, 1005);
				regmap_write(switch_priv->regmap, DIO4480_REG_SWITCH_SELECT, 0x00);//GND - GSBU1, MIC - SBU2
				regmap_write(switch_priv->regmap, DIO4480_REG_SWITCH_SETTINGS, 0x9f);
				usleep_range(10000, 10005);
			...
			if (jack_status == 0x01) {
				...
				regmap_write(switch_priv->regmap, DIO4483_REG_RESET, reg_val);//reset DIO4483
				usleep_range(1000, 1005);
				regmap_write(switch_priv->regmap, DIO4483_REG_SWITCH_SELECT, 0x00);//GND - GSBU1, MIC - SBU2
			...
			if (jack_status == 0x01) {
				...
				ret = regmap_write(switch_priv->regmap, DEFAULT_REG_SWITCH_SELECT, 0x00);//GND - GSBU1, MIC - SBU2
				if (ret != 0) {
					dev_err(dev, "%s, %d, regmap fail!\n", __func__, __LINE__);
					goto regmap_err;
				}
		...
		if (switch_priv->vendor == DIO4480) {
			regmap_write(switch_priv->regmap, DIO4480_REG_RESET, 0x01);//reset DIO4480
			usleep_range(1000, 1005);
			regmap_write(switch_priv->regmap, DIO4480_REG_SWITCH_SELECT, 0x18);
			regmap_write(switch_priv->regmap, DIO4480_REG_SWITCH_SETTINGS, 0x98);
			...
			ret = regmap_write(switch_priv->regmap, DEFAULT_REG_RESET, reg_val);//reset WAS4783
			if (ret != 0) {
				dev_err(dev, "%s, %d, regmap fail!\n", __func__, __LINE__);
				goto regmap_err;
			}
	...
	case TYPEC_SWITCH_MIC_GND_SWAP:
		if (switch_priv->vendor == DIO4480) {
			if (typec_switch_status) {
				...
				regmap_write(switch_priv->regmap, DIO4483_REG_RESET, 0x01);
				usleep_range(1000, 1005);
				regmap_write(switch_priv->regmap, DIO4483_REG_SWITCH_SELECT, 0x07); //GND - GSBU2, MIC - SBU1
				regmap_write(switch_priv->regmap, DIO4483_REG_SWITCH_SETTINGS, 0x9f);
				typec_switch_status = 0;
			}
		} else if (switch_priv->vendor == DIO4483) {
			if (typec_switch_status) {
				...
				SET_BIT(reg_val, DIO4483_I2C_RESET);
				dev_info(switch_priv->dev, "%s, %d, reg_val = 0x%02x", __func__, __LINE__, reg_val);
				regmap_write(switch_priv->regmap, DIO4483_REG_RESET, reg_val);
				usleep_range(1000, 1005);
			...
			if (typec_switch_status) {
				// TODO Readonly?
				regmap_write(switch_priv->regmap, DEFAULT_REG_SWITCH_SELECT, 0x07); //GND - GSBU2, MIC - SBU1
...
static void dio4483_update_reg_defaults(struct regmap *regmap)
{
	...
	for (i = 0; i < ARRAY_SIZE(dio4483_i2c_reg); i++) {
		regmap_write(regmap, dio4483_i2c_reg[i].reg,
...
static void default_update_reg_defaults(struct regmap *regmap)
{
	...
	for (i = 0; i < ARRAY_SIZE(default_i2c_reg); i++) {
		regmap_write(regmap, default_i2c_reg[i].reg,
			 ...
			 const struct i2c_device_id *id)
{
	...
	SET_BIT(reg_val, DEFAULT_I2C_RESET);
	dev_info(dev, "%s, %d, reg_val = 0x%02x", __func__, __LINE__, reg_val);
	regmap_write(switch_priv->regmap, DEFAULT_REG_RESET, reg_val);//reset DIO4483
	...
	if (switch_priv->vendor == WAS4783 || switch_priv->vendor == FSA4480) {
...
static void typec_switch_shutdown(struct i2c_client *i2c) {
	...
	if (switch_priv->vendor == DIO4480) {
		regmap_write(switch_priv->regmap, DIO4480_REG_RESET, 0x01);
	} else if (switch_priv->vendor == DIO4483) {
		regmap_write(switch_priv->regmap, DIO4483_REG_RESET, 0x01);
	} else {
		default_update_reg_defaults(switch_priv->regmap);
