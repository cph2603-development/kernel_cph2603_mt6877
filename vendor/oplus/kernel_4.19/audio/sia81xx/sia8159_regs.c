...
extern int en_peak_current_limit;
static void sia8159_check_trimming(
	struct regmap *regmap)
{
	static char defaults[reg_num] = {0x76, 0x66, 0x70};
	uint8_t vals[reg_num] = {0};
	uint8_t vals_ops[reg_num] = {0};
	uint8_t crc = 0;
	uint8_t i, tmp;
	...
	if (en_peak_current_limit)
		pr_info("[ info][%s] %s: en_peak_current_limit enable !!! \r\n",
			LOG_FLAG, __func__);
	...
	if (crc != crc4_itu(vals, reg_num)) {
	...
	} else if (en_peak_current_limit) {
		pr_info("[ info][%s] %s: trimming ops \r\n",
			LOG_FLAG, __func__);
		...
		*vals |= 0x02;
		...
		if (vals_ops[2] >= 0x20)
			vals_ops[2] = vals_ops[2] - 0x20;
		else
			vals_ops[2] &= 0x0F;

		sia81xx_regmap_write(regmap,
			SIA8159_REG_TRIMMING_BEGIN, reg_num, vals_ops);
