...
#define EPOLAR	(101) /* pulse electrical level oplus site with the polarity */
	...
	struct sia81xx_dev_s *sia81xx)
{
	...
	if(0 == sia81xx->disable_pin && sia81xx->is_sia_pa) {
	...
	if (is_chip_type_supported(sia81xx->chip_type) && sia81xx->is_sia_pa)
	{
	...
	struct device_node	*sia81xx_of_node)
{
	...
	ret = of_property_read_u32(sia81xx_of_node, "en_peak_current_limit", &en_peak_current_limit);
	if((0 != ret) || (1 != en_peak_current_limit)) {
		en_peak_current_limit = 0;
	}
	pr_debug("[debug][%s] %s: running en_peak_current_limit = %d \r\n", LOG_FLAG, __func__, en_peak_current_limit);
...
#endif /* OPLUS_BUG_COMPATIBILITY */
	if (CHIP_TYPE_SIA8159 == g_sia81xx->chip_type) {
		pr_info("[ info][%s] %s: SIA8159 does not need voltage follows volume change \r\n", LOG_FLAG, __func__);
		return 0;
	}
	...
	const struct i2c_device_id *id)
{
	...
	if (((sia81xx_regmap_check_chip_id(sia81xx->regmap, sia81xx->chip_type) == 0)
		...
		sia81xx->is_sia_pa = false;
		// there is SIAxxx, we only need the speaker protection algorithm
		if ((sia81xx->channel_num == 0) && (speaker_device != NULL)) {
		...
		pr_info("%s():for SIAxx,oplus_register end\r\n", LOG_FLAG, __func__);
	}
...
static int sia81xx_probe(struct platform_device *pdev)
{
	...
	pr_info("[ info][%s]  %s: line = %d  disable_pin  = %d\r\n", LOG_FLAG, __func__,__LINE__,disable_pin);
	if(0 != ret) {
	...
	if(0 == disable_pin) {
		...
		pr_info("[ info][%s]  %s: line = %d  rst_pin  = %d\r\n", LOG_FLAG, __func__,__LINE__,rst_pin);
		if(rst_pin < 0) {
		...
		pr_info("[ info][%s]  %s: line = %d  owi_pin  = %d\r\n", LOG_FLAG, __func__,__LINE__,owi_pin);
...
#ifdef CONFIG_SND_SOC_OPLUS_PA_MANAGER
	if(CHIP_TYPE_SIA8102 == sia81xx->chip_type){
		speaker_device = get_speaker_dev(sia81xx->channel_num + 1);
		pr_info("%s():,oplus_register start\r\n", LOG_FLAG, __func__);
		if (speaker_device == NULL) {
			speaker_device = kzalloc(sizeof(struct oplus_speaker_device), GFP_KERNEL);
