...
#define AW87XXX_DEV_LEFT 0
#define AW87XXX_DEV_RIGHT 1
 ...
 *
 ************************************************************************/
static void aw87xxx_update_voltags_max(struct aw87xxx *aw87xxx,
				struct aw_data_container *data_container)
{
	int i = 0;
	struct aw_voltags_desc *vol_desc = &aw87xxx->aw_dev.vol_desc;
	...
	for (i = 0; i < data_container->len; i = i + 2) {
		if (data_container->data[i] == vol_desc->addr) {
			...
			AW_DEV_LOGD(aw87xxx->dev, "get voltags max = 0x%02x",
...
static int aw87xxx_power_on(struct aw87xxx *aw87xxx, char *profile)
{
	...
	aw87xxx_update_voltags_max(aw87xxx, data_container);
...
char *aw87xxx_show_current_profile(int dev_index)
{
	...
	list_for_each(pos, &g_aw87xxx_list) {
		...
		if (aw87xxx->dev_index == dev_index) {
			AW_DEV_LOGI(aw87xxx->dev, "current profile is [%s]", aw87xxx->current_profile);
...
int aw87xxx_set_profile(int dev_index, char *profile)
{
	...
	list_for_each(pos, &g_aw87xxx_list) {
		...
		if (profile && aw87xxx->dev_index == dev_index) {
			AW_DEV_LOGD(aw87xxx->dev, "set dev_index = %d, profile = %s", dev_index, profile);
...
int aw87xxx_set_boost_voltage(int dev_index, int status)
{
	...
	struct list_head *pos = NULL;
	struct aw87xxx *aw87xxx = NULL;
	struct aw_voltags_desc *vol_desc = NULL;
	...
	list_for_each(pos, &g_aw87xxx_list) {
		...
		if (aw87xxx->dev_index == dev_index) {
			...
			if ((aw87xxx->aw_dev.chipid != AW_DEV_CHIPID_76) &&
				(aw87xxx->aw_dev.chipid != AW_DEV_CHIPID_39) &&
				(aw87xxx->aw_dev.chipid != AW_DEV_CHIPID_18)) {
				AW_DEV_LOGE(aw87xxx->dev, "chipid[0x%02x] not set voltage!",
			...
			if (status == AW_VOLTAGS_HIGH) {
				AW_DEV_LOGI(aw87xxx->dev, "set reg voltags max: 0x%02x = 0x%02x",
					vol_desc->addr, vol_desc->vol_max);
				ret = aw87xxx_dev_i2c_write_byte(&aw87xxx->aw_dev,
					vol_desc->addr, vol_desc->vol_max);
				if (ret < 0)
					return ret;
			} else if (status == AW_VOLTAGS_LOW) {
				AW_DEV_LOGI(aw87xxx->dev, "set reg voltags min: 0x%02x = 0x%02x",
...
int aw87xxx_get_boost_voltage(int dev_index, int *status)
{
	...
	list_for_each(pos, &g_aw87xxx_list) {
		...
		if (aw87xxx->dev_index == dev_index) {
			if (aw87xxx->aw_dev.chipid != AW_DEV_CHIPID_76) {
				AW_DEV_LOGE(aw87xxx->dev, "chipid[0x%02x] not get voltage!",
					aw87xxx->aw_dev.chipid);
				return -EINVAL;
			}
			if (0 == strncmp(aw87xxx->current_profile, aw87xxx->prof_off_name,
			...
			ret = aw87xxx_dev_i2c_read_byte(&aw87xxx->aw_dev,
					vol_desc->addr, &reg_voltags_val);
				if (ret < 0)
					return ret;

			if (reg_voltags_val ==  vol_desc->vol_max) {
				*status = AW_VOLTAGS_HIGH;
			} else if (reg_voltags_val ==  vol_desc->vol_min) {
				*status = AW_VOLTAGS_LOW;
			} else {
...
static int aw87xxx_dev_set_vol_reg_normal(struct aw87xxx *aw87xxx)
{
	...
	ret = aw87xxx_dev_i2c_write_byte(&aw87xxx->aw_dev, data_container->data[4],
				data_container->data[5]);
	if (ret < 0){
		AW_DEV_LOGE(aw87xxx->dev, "set reg:%x = %x FAIL",data_container->data[4],data_container->data[5]);
	}
...
static void aw87xxx_audio_spk_low_voltage_status(int bStatus)
{
	struct aw87xxx *aw87xxx = NULL;
	struct list_head *pos = NULL;
	struct aw_prof_info *prof_info = NULL;
	//struct aw_data_container *data_container = NULL;

	aw87xxx_2_x_0_spk_low_voltage_status = bStatus;
	...
	list_for_each(pos, &g_aw87xxx_list) {
		...
		if (aw87xxx_2_x_0_spk_low_voltage_status > 0) {
			pr_info("%s: aw87xxx write vol reg Low VOL when playing\n", __func__);
			//set as 7V
			aw87xxx_dev_i2c_write_byte(&aw87xxx->aw_dev, 0x03, aw87xxx->low_volume_voltage);
		} else {
			pr_info("%s: aw87xxx write vol reg normal when playing\n", __func__);
			if (!prof_info->status) {
				AW_DEV_LOGE(aw87xxx->dev, "profile_cfg not load,set default:0x08-(8V)");
				aw87xxx_dev_i2c_write_byte(&aw87xxx->aw_dev, 0x03, AW87XXX_HIGH_VOLUME_VOLTAGE_DEFAULT);	//set as normal:8V
			} else {
				AW_DEV_LOGE(aw87xxx->dev, "aw87xxx write vol reg normal by profile_cfg\n");
				aw87xxx_dev_i2c_write_byte(&aw87xxx->aw_dev, 0x03, AW87XXX_HIGH_VOLUME_VOLTAGE_DEFAULT);	//FUNC-1:use default to set as normal:8V
				//aw87xxx_set_profile(AW_DEV_0, aw_profile[0]);		//FUNC-2:when volum up ,music Caton & POP
				//aw87xxx_dev_set_vol_reg_normal(aw87xxx);			//FUNC-3:no sound;
			}
		}
	}
}
...
static void aw87xxx_fw_load_work_routine(struct work_struct *work)
{
	...
	if (prof_info->status == AW_ACF_WAIT) {
		ret = request_firmware_nowait(THIS_MODULE,
				...
				aw87xxx_fw_load);
		if (ret != 0)
			AW_DEV_LOGE(aw87xxx->dev, "request firmware failed");
	}
}
...
static ssize_t aw87xxx_attr_dev_type_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	struct aw87xxx *aw87xxx = dev_get_drvdata(dev);
	...
	len += snprintf(buf + len, PAGE_SIZE - len,
		"%s\n", aw87xxx->dev_index?"R_SPK":"L_SPK");
	AW_DEV_LOGI(aw87xxx->dev, "%s", buf);
	return len;
}
...
static DEVICE_ATTR(dev_type, S_IRUGO, aw87xxx_attr_dev_type_show, NULL);
				...
				struct device_node *dev_node)
{
	int pcb_version = -1;
	int ret = -1;
	const char *aw87xxx_prj = NULL;
	bool isLijingA = false;
	int8_t voltags_min = -EINVAL;
	int32_t dev_index = -EINVAL;
	int32_t low_volume_voltage = -EINVAL;
	struct aw_voltags_desc *vol_desc = &aw87xxx->aw_dev.vol_desc;

	ret = of_property_read_string_index(dev_node, "aw,aw87xxx_prj", 0, &aw87xxx_prj);
	if (0 != ret) {
	    isLijingA = false;
	    AW_DEV_LOGI(aw87xxx->dev, "not found  aw,aw87xxx_prj, ret=%d", ret);
	} else {
	    AW_DEV_LOGI(aw87xxx->dev, "found  aw,aw87xxx_prj, ret=%d", ret);
	    if (!strncmp(aw87xxx_prj, "lijing-a", strlen("lijing-a"))) {
	        isLijingA = true;
	    }
	}
	...
	if (ret < 0) {
		...
		if (isLijingA) {
		    ...
		    if (pcb_version < 5) {
		        AW_DEV_LOGI(aw87xxx->dev, "Set Spk AW87XXX_DEV_RIGHT");
		        dev_index = AW87XXX_DEV_RIGHT;
		    } else {
		        AW_DEV_LOGI(aw87xxx->dev, "Set Spk AW87XXX_DEV_LEFT");
		        dev_index = AW87XXX_DEV_LEFT;
		    }
		}
	...
	ret = of_property_read_u8(dev_node, "aw-voltags-min", &voltags_min);
	if (ret < 0) {
		AW_DEV_LOGI(aw87xxx->dev, "voltags_min parse failed, user default[0x%02x], ret=%d",
				AW_BOOST_VOLTAGS_MIN, ret);
		vol_desc->vol_min = AW87XXX_LOW_VOLUME_VOLTAGE;
	} else {
		vol_desc->vol_min = voltags_min;
		AW_DEV_LOGI(aw87xxx->dev, "parse aw_voltags_min=[0x%02x]",
				vol_desc->vol_min);
	}
	return 0;
}
