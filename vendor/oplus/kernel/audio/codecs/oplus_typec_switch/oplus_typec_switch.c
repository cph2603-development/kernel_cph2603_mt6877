...
static int typec_switch_usbc_analog_setup_switches(struct typec_switch_priv *switch_priv)
{
	...
	if (switch_priv->plug_state) {
		...
		if (gpio_is_valid(switch_priv->hs_det_pin)) {
			...
			#if IS_ENABLED(CONFIG_OPLUS_MTK_HEADSET_RECHECK)
			/* Add for check cable state after plug in/out 1s and force accdet irq. */
			schedule_delayed_work(&switch_priv->hp_plugin_check_work, msecs_to_jiffies(1000));
			#endif /* CONFIG_OPLUS_MTK_HEADSET_RECHECK */
		}
	} else {
		...
		if (gpio_is_valid(switch_priv->hs_det_pin)) {
			...
			#if IS_ENABLED(CONFIG_OPLUS_MTK_HEADSET_RECHECK)
			/* Add for check cable state after plug in/out 1s and force accdet irq. */
			schedule_delayed_work(&switch_priv->hp_plugout_check_work, msecs_to_jiffies(1000));
			#endif /* CONFIG_OPLUS_MTK_HEADSET_RECHECK */
		}
